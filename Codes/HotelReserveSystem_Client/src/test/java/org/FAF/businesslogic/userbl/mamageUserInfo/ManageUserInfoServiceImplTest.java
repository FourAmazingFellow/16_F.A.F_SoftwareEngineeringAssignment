...
public class ManageUserInfoServiceImplTest {
	...
	public void setUp() throws Exception {
		...
		this.userIDNew = "wmarketstaff0001";
		this.passwordNew = "claris123";
		this.telNumNew = "12345678909";
		this.userTypeNew = UserType.WebMarketStaff;
	...
	public void testGetHotelStaffInfo() {
		...
		try {
			...
			assertEquals("ManageUserInfoService.getUserInfo(userID,userType) has an error in userID!", userID,
					hotelStaffInfoVO.userID);
			assertEquals("ManageUserInfoService.getUserInfo(userID,userType) has an error in password!", password,
					hotelStaffInfoVO.password);
			assertEquals("ManageUserInfoService.getUserInfo(userID,userType) has an error in telNum!", telNum,
					hotelStaffInfoVO.telNum);
			assertEquals("ManageUserInfoService.getUserInfo(userID,userType) has an error in hotelAddress!",
					hotelAddress, hotelStaffInfoVO.hotelAddress);
		} catch (RemoteException e) {
			e.printStackTrace();
			fail();
		}
	...
	public void testGetUserInfo() {
		...
		try {
			...
			assertEquals("ManageUserInfoService.getUserInfo(userID,userType) has an error in userID!", userIDw,
					userVO.userID);
			assertEquals("ManageUserInfoService.getUserInfo(userID,userType) has an error in password!", passwordw,
					userVO.password);
			assertEquals("ManageUserInfoService.getUserInfo(userID,userType) has an error in telNum!", telNumw,
					userVO.telNum);
		} catch (RemoteException e) {
			e.printStackTrace();
			fail();
